...
export function checkBallCollisions(ball, player1, player2, winScore) {
  const result = {
    paddleHit: null,
    scored: null,
    gameEnded: false,
  ...
  if (ball.x <= 0) {
    ...
    ball.vx = PHYSICS.BALL_INITIAL_VX; // Reset to default speed going right
    ball.x = 0;
  ...
  if (ball.x >= 100) {
    ...
    ball.vx = -PHYSICS.BALL_INITIAL_VX; // Reset to default speed going left
    ball.x = 100;
